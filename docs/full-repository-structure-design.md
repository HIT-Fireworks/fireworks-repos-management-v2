# HIT-Fireworks 仓库模型与历史设计记录

## 当前生效契约（正式切换）

仓库是唯一资源边界。完整课程代码直接且唯一归属 `repo_id` 与 `physical_repository_id`；文件通过 `repo_id/path` 唯一定位。当前模型没有 `resource_groups`、`resource_group_id`、`member_resource_group_ids` 或 `component_id`。`CourseGroup`、教学记录的模块/方向/来源章节和 curriculum history 保留为教学语义，不参与资源分桶。

新增代码必须由管理员显式分配，不按同名自动获得资料。拆分必须完整分配全部代码与文件，共享文件关联代码不可跨目标；无代码文件明确指定去向，无资料代码可以独立拆分。普通独立资料在根目录，真实软件包及多文件文档结构保留。合并/拆分保持真实相对路径；同路径冲突先明确解决，不生成分桶容器，不丢弃或静默覆盖字节。冻结源树中的未清点文件必须保留或明确随迁。

Rust 是唯一发行运行时，沿用现有三份 canonical 文件路径与分片机制。旧资源组配置、Python 迁移生成器及下文原设计均仅为冻结历史证据，不能用于重建当前生产模型。

---

以下完整保留旧设计与当时审计数据，**已被上述契约取代，不是现行操作指引**。
